const KW = (source: string) => new Set(source.split(/\s+/).filter(Boolean))
...
function resolve(lang: string): LangSpec | null {
  return LANGS[ALIAS[lang] ?? lang] ?? null
}
...
export function highlightTokens(line: string, lang: string): Array<[string, string]> {
  ...
  for (const match of line.matchAll(TOKEN_RE)) {
    ...
    if (start > last) tokens.push(['', line.slice(last, start)])
    const tok = match[0] || ''
    const ch = tok[0] || ''
    if (ch === '"' || ch === '\'' || ch === '`') tokens.push(['str', tok])
    else if (ch >= '0' && ch <= '9') tokens.push(['num', tok])
    else if (spec.keywords.has(tok)) tokens.push(['kw', tok])
